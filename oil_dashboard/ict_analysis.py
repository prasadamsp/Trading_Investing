# =============================================================================
# Oil Weekly Bias Dashboard — ICT Analysis Engine
# =============================================================================
"""
Applies Inner Circle Trader (ICT) concepts algorithmically to crude oil (WTI) price data
...
def find_swing_points(df: pd.DataFrame, order: int = 3) -> dict:
    """
...
def detect_market_structure(df: pd.DataFrame, lookback: int = 10,
    ...
    Args:
        order: fractal window passed to find_swing_points(); defaults to
               config.ICT_SWING_ORDER when None.

    Returns: 'bullish' | 'bearish' | 'ranging'
    """
    eff_order = order if order is not None else config.ICT_SWING_ORDER
    swings = find_swing_points(df, order=eff_order)
...
def generate_ict_trades(monthly_df: pd.DataFrame,
    ...
    Args:
        ...
        params:     Optional dict of parameter overrides for backtesting /
                    optimization. Keys match config.ICT_* names. When None,
                    all values fall back to config.py defaults — identical
                    to the original behaviour when called from app.py.
    ...
    # ── Parameter resolution: backtest overrides or config defaults ───────
    def _p(key: str, default):
        return params[key] if params and key in params else default

    swing_lookback   = _p("ICT_DAILY_SWING_LOOKBACK",      config.ICT_DAILY_SWING_LOOKBACK)
    swing_fallback   = _p("ICT_DAILY_SWING_FALLBACK",       config.ICT_DAILY_SWING_FALLBACK)
    swing_min_range  = _p("ICT_DAILY_SWING_MIN_RANGE_PCT",  config.ICT_DAILY_SWING_MIN_RANGE_PCT)
    bias_bull_thresh = _p("ICT_BIAS_BULL_THRESHOLD",        config.ICT_BIAS_BULL_THRESHOLD)
    bias_bear_thresh = _p("ICT_BIAS_BEAR_THRESHOLD",        config.ICT_BIAS_BEAR_THRESHOLD)
    conf_thresh      = _p("ICT_CONFIDENCE_HIGH_THRESHOLD",  config.ICT_CONFIDENCE_HIGH_THRESHOLD)
    ob_near_lu       = _p("ICT_OB_NEAR_LONG_UPPER",         config.ICT_OB_NEAR_LONG_UPPER)
    ob_near_ll       = _p("ICT_OB_NEAR_LONG_LOWER",         config.ICT_OB_NEAR_LONG_LOWER)
    ob_near_sl_      = _p("ICT_OB_NEAR_SHORT_LOWER",        config.ICT_OB_NEAR_SHORT_LOWER)
    ob_near_su       = _p("ICT_OB_NEAR_SHORT_UPPER",        config.ICT_OB_NEAR_SHORT_UPPER)
    ob_stop_frac     = _p("ICT_OB_STOP_BUFFER_FRACTION",    config.ICT_OB_STOP_BUFFER_FRACTION)
    ob_stop_fb       = _p("ICT_OB_STOP_BUFFER_FALLBACK",    config.ICT_OB_STOP_BUFFER_FALLBACK)
    liq_stop_pct     = _p("ICT_LIQ_STOP_PCT",               config.ICT_LIQ_STOP_PCT)
    fvg_lookback     = _p("ICT_FVG_LOOKBACK",               config.ICT_FVG_LOOKBACK)
    ob_lookback      = _p("ICT_OB_LOOKBACK",                config.ICT_OB_LOOKBACK)
    ob_min_impulse   = _p("ICT_OB_MIN_IMPULSE",             config.ICT_OB_MIN_IMPULSE)
    ote_low_key      = _p("ICT_OTE_LOW",                    config.ICT_OTE_LOW)
    ote_high_key     = _p("ICT_OTE_HIGH",                   config.ICT_OTE_HIGH)
    swing_order      = _p("ICT_SWING_ORDER",                config.ICT_SWING_ORDER)
    atr_period       = _p("ICT_ATR_PERIOD",                 config.ICT_ATR_PERIOD)
    ote_atr_mult     = _p("ICT_OTE_ATR_MULTIPLIER",         config.ICT_OTE_ATR_MULTIPLIER)
    liq_atr_mult     = _p("ICT_LIQ_ATR_MULTIPLIER",         config.ICT_LIQ_ATR_MULTIPLIER)
    ote_stop_fb_pct  = _p("ICT_OTE_STOP_FALLBACK_PCT",      config.ICT_OTE_STOP_FALLBACK_PCT)
    liq_stop_fb_pct  = _p("ICT_LIQ_STOP_FALLBACK_PCT",      config.ICT_LIQ_STOP_FALLBACK_PCT)
    # liq_stop_pct retained in resolver for compatibility; superseded by ATR stops

    # ── Compute building blocks ───────────────────────────────────────────
    try:
        monthly_structure = detect_market_structure(monthly_df, order=swing_order)
        weekly_structure  = detect_market_structure(weekly_df,  order=swing_order)
        key_levels        = get_key_levels(monthly_df, weekly_df)
        current_price     = float(daily_df["Close"].iloc[-1])
        # ATR for dynamic stop sizing — shared by Trade 2 and Trade 3
        atr = indicators.calc_atr(daily_df, period=atr_period)

        # Use DAILY swing for fibonacci so OTE zone stays near current price.
        # swing_lookback-bar window captures the most recent local swing rather
        # than the macro multi-year move which would put OTE far from price.
        sh, sl, swing_dir = _find_major_swing(daily_df, lookback_bars=swing_lookback)
        # Fallback: if daily range is too tight, widen to swing_fallback bars
        if sh - sl < current_price * swing_min_range:
            sh, sl, swing_dir = _find_major_swing(daily_df, lookback_bars=swing_fallback)

        fib       = calc_fibonacci_levels(sh, sl)
        fifty_pct = fib.get(0.5)

        weekly_obs  = [o for o in find_order_blocks(weekly_df, n_recent=ob_lookback,
                                                    min_impulse_pct=ob_min_impulse)
                       if o["valid"]]
        daily_obs   = [o for o in find_order_blocks(daily_df,  n_recent=ob_lookback,
                                                    min_impulse_pct=ob_min_impulse)
                       if o["valid"]]
        weekly_fvgs = [f for f in find_fvgs(weekly_df, n_recent=fvg_lookback) if not f["filled"]]
        daily_fvgs  = [f for f in find_fvgs(daily_df,  n_recent=fvg_lookback) if not f["filled"]]
    ...
    if (monthly_structure == "bullish"
            ...
            and bias_score > bias_bull_thresh):
        overall_bias = "bullish"
    elif (monthly_structure == "bearish"
            and weekly_structure == "bearish"
            and bias_score < bias_bear_thresh):
    ...
    def _trade1() -> dict:
        ...
        if overall_bias == "bullish":
            # Find nearest valid bullish OB within proximity range of current price.
            # Sort by proximity (closest high to current price first).
            candidates = sorted(
                [o for o in (daily_obs + weekly_obs)
                 if o["direction"] == "bullish"
                 and o["high"] <= current_price * ob_near_lu
                 and o["high"] >= current_price * ob_near_ll],
            ...
            if ob_used:
                ...
                stop   = round(ob_used["low"] - max(ob_range * ob_stop_frac,
                                                    current_price * ob_stop_fb), 1)
                conf   = "HIGH" if abs(bias_score) > conf_thresh else "MEDIUM"
            ...
            candidates = sorted(
                [o for o in (daily_obs + weekly_obs)
                 ...
                 and o["low"] >= current_price * ob_near_sl_
                 and o["low"] <= current_price * ob_near_su],
            ...
            if ob_used:
                ob_range = ob_used["high"] - ob_used["low"]
                entry  = round((ob_used["high"] + ob_used["low"]) / 2, 1)
                stop   = round(ob_used["high"] + max(ob_range * ob_stop_frac,
                                                     current_price * ob_stop_fb), 1)
                conf   = "HIGH" if abs(bias_score) > conf_thresh else "MEDIUM"
    ...
    def _trade2() -> dict:
        ...
        # Compute OTE prices directly from swing levels so any float fib value
        # works (not just the pre-computed ICT_FIB_LEVELS keys in the dict).
        _sh  = fib["swing_high"]
        _rng = fib["range"]
        ote_low  = round(_sh - ote_low_key  * _rng, 2)
        ote_high = round(_sh - ote_high_key * _rng, 2)
        ...
        if overall_bias == "bullish":
            ...
            _fib786 = fib.get(0.786, ote_high * (1 - ote_stop_fb_pct))
            _buf    = (atr * ote_atr_mult) if atr else (current_price * ote_stop_fb_pct)
    ...
    def _trade3() -> dict:
        ...
        if overall_bias == "bullish":
            ...
            entry  = round(current_price, 1)
            _buf   = (atr * liq_atr_mult) if atr else (current_price * liq_stop_fb_pct)
            stop   = round(entry + _buf, 1)
            ...
            if current_price > mid:
                ...
                _buf   = (atr * liq_atr_mult) if atr else (entry * liq_stop_fb_pct)
                stop   = round(entry + _buf, 1)
                tp1    = round(mid, 1)
                tp2    = round(pwl, 1)
                return {
