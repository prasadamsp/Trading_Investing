...
def main():
    ...
    if monthly_df_ict.empty or weekly_df_ict.empty or daily_df_ict.empty:
        ...
        # Backtest performance labels per confidence level (from backtest.py findings)
        _BT_LABEL = {
            "HIGH":   ("✓ Backtest: 100% TP1  E=+1.93", "#00C853"),
            "MEDIUM": ("✓ Backtest:  75% TP1  E=+1.87", "#69F0AE"),
            "LOW":    ("⚠ Backtest:   8% TP1  E=-0.49", "#FF6D00"),
        }
        ...
        for col, trade, title in zip([tc1, tc2, tc3], ict_trades, trade_titles):
            ...
            # Grey-out styling for LOW confidence cards
            card_opacity  = "0.55" if is_low else "1.0"
            card_border   = "1px solid #444" if is_low else ""
            card_style    = (f"opacity:{card_opacity}; border:{card_border};"
                             if is_low else "")

            bt_text, bt_color = _BT_LABEL.get(conf, ("", "#888"))
            ...
            with col:
                ...
                <div class="metric-card" style="{card_style}">
                    ...
                    <div style="font-size:10px; color:{bt_color}; margin-bottom:4px;
