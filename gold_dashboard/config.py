...
ICT_SWING_ORDER       = 4     # bars each side to confirm a swing point (fractal) [optimised: was 3]
ICT_FVG_LOOKBACK      = 20    # candles back to scan for Fair Value Gaps
ICT_OB_LOOKBACK       = 20    # candles back to scan for Order Blocks
ICT_OB_MIN_IMPULSE    = 0.3   # % move that qualifies as an impulse after an OB [optimised: was 0.5]
...
# ---------------------------------------------------------------------------
# ICT Trade Generation — tunable thresholds (extracted from ict_analysis.py)
# Defaults match the original hardcoded values; override via params dict in
# generate_ict_trades() for backtesting / optimization.
# ---------------------------------------------------------------------------
...
ICT_BIAS_BULL_THRESHOLD       = 0.05    # bias_score > this  → bullish overall_bias [optimised: was 0.10]
ICT_BIAS_BEAR_THRESHOLD       = -0.05   # bias_score < this  → bearish overall_bias [optimised: was -0.10]
ICT_CONFIDENCE_HIGH_THRESHOLD = 0.30    # abs(bias_score) > this → "HIGH" confidence
ICT_OB_NEAR_LONG_UPPER        = 1.03    # bullish OB filter: ob.high ≤ price × this
ICT_OB_NEAR_LONG_LOWER        = 0.92    # bullish OB filter: ob.high ≥ price × this
ICT_OB_NEAR_SHORT_LOWER       = 0.97    # bearish OB filter: ob.low  ≥ price × this
ICT_OB_NEAR_SHORT_UPPER       = 1.08    # bearish OB filter: ob.low  ≤ price × this
ICT_OB_STOP_BUFFER_FRACTION   = 0.50    # stop buffer = ob_range × this
ICT_OB_STOP_BUFFER_FALLBACK   = 0.003   # fallback stop buffer = price × this
ICT_LIQ_STOP_PCT              = 0.005   # Trade 3 stop distance = price × this (superseded by ATR stops below)
ICT_ATR_PERIOD                = 14      # ATR period for dynamic stop sizing
ICT_OTE_ATR_MULTIPLIER        = 1.5     # Trade 2 stop buffer = ATR × this [replaces fixed $5]
ICT_LIQ_ATR_MULTIPLIER        = 1.0     # Trade 3 stop buffer = ATR × this [replaces fixed 0.5%]
ICT_OTE_STOP_FALLBACK_PCT     = 0.006   # Trade 2 fallback stop buffer if ATR unavailable
ICT_LIQ_STOP_FALLBACK_PCT     = 0.004   # Trade 3 fallback stop buffer if ATR unavailable
